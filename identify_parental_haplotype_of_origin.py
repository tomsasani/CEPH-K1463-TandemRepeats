...
def main(args):
    ...
    informative_sites = pd.read_csv(
        args.annotated_dnms, sep="\t", dtype={"sample_id": str}
    )
    InformativeSiteSchema.validate(informative_sites)

    # file handles for (p/m)aternal SNV and STR VCFs (phased)
    dad_str_vcf = VCF(
        f"{args.hiphase_pref}/{args.dad_id_orig}_{args.assembly}_50bp_merge.sorted.phased.vcf.gz"
    )
    dad_snv_vcf = VCF(
        f"{args.hiphase_pref}/{args.dad_id}.{args.assembly}.deepvariant.glnexus.phased.vcf.gz"
    )
    mom_str_vcf = VCF(
        f"{args.hiphase_pref}/{args.mom_id_orig}_{args.assembly}_50bp_merge.sorted.phased.vcf.gz"
    )
    mom_snv_vcf = VCF(
        f"{args.hiphase_pref}/{args.mom_id}.{args.assembly}.deepvariant.glnexus.phased.vcf.gz"
    )
