...
def plot_phasing(df: pd.DataFrame):
    ...
    for origin in (0, 1):
        ...
        ax.scatter(hap_a["inf_pos"], [0] * hap_a.shape[0], c="red" if origin == 0 else "blue")
        ax.scatter(hap_b["inf_pos"], [1] * hap_b.shape[0], c="red" if origin == 0 else "blue")
    ax.set_yticks([0, 1])
    ax.set_yticklabels(["haplotype A", "haplotype_B"])
    return f, ax
...
def main(args):
    ...
    for _, row in tqdm.tqdm(mutations.iterrows()):
        ...
        for var in KID_STR_VCF(f"{trid_chrom}:{trid_start}-{trid_end}"):
            ...
            # ax.scatter(str_midpoint, 0 if denovo_hap_id == "A" else 1)
            # f.savefig(f"fig/{trid}.png")
            # plt.close()
