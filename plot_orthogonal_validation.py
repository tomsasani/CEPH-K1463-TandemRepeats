...
mutations = []
for fh in glob.glob(f"tr_validation/downsampled/csv/orthogonal_support/*.{ASSEMBLY}.{ORTHOGONAL_TECH}.tsv"):
...
# mutations = mutations[mutations["sample_id"] == "200087"]

# FILTERING
mutations = mutations[np.abs(mutations["likely_denovo_size"]) >= MIN_SIZE]

# mutations = mutations[mutations["simple_motif_size"] == "STR"]
mutations = mutations[mutations["validation_status"] == "pass"]

print (mutations[["#chrom", "start", "end", "child_AL", "father_AL", "mother_AL", "phase_summary"]].head(20))
